...
public class HierarchicalBCEngine implements GraphConstants, LayoutEngine {
	...
	public void fireLayoutCompleteEvent(LayoutCompleteEvent e) {
		if (layoutCompleteListeners != null && layoutCompleteListeners.size() != 0) {
			...
			for (LayoutCompleteEventListener iterator : layoutCompleteListeners) {
				l = iterator;
				l.layoutCompleted(e);
			}
		}
		...
		Thread th = new Thread() {
			...
			public void run() {
				...
				if (m_jRbTopdown.isSelected()) {
					int crossbefore = crossings(nodeLevels);
					int crossafter = 0;
					int i = 0;
					do {
	...
	protected void makeProperHierarchy() {
		...
		for (int i = 0; i < nodesLevel.length; i++) {
			if (nodesLevel[i] == 0) {
				...
				// if the shallowest child of a parent has a depth greater than
				// 1
		...
		for (int iterator : nodesLevel) {
			levelCounts[iterator]++;
		}
		// System.out.println("------------------------------------------");
		...
		// *Making a proper Hierarchy by putting in dummy vertices to close all
		// gaps
	...
	private void removeGaps(int nodesLevel[]) {
		...
		int temp2 = graphMatrix[0].length;
		int tempCnt = 1;
			...
			for (int i = 0; i < temp2; i++) {
				...
				if (graphMatrix[n][i] > 0) {
					if (nodesLevel[i] > nodesLevel[n] + 1) {
						...
						level++;
						// nodeLevels[level++].addElement(new
						// Integer(m_nodes.size()-1));
						...
						// "+((GraphNode)m_nodes.get(k)).ID+
						// " i "+((GraphNode)m_nodes.get(i)).ID+
						// " n "+((GraphNode)m_nodes.get(n)).ID+
						// " len "+((GraphNode)m_nodes.get(len)).ID );
						// temp[origNode][firstTempNodecreated] =
						// temp[origNode][targetNode]
						tempMatrix[n][len] = tempMatrix[n][i];
						// temp[firstTempNodeCreated][origNode] for reverse
						// tracing
						tempMatrix[len][n] = -1 * tempMatrix[n][i];
						// temp[targetNode][lastTempNodecreated] for reverse
						// tracing
						tempMatrix[i][k] = -1 * tempMatrix[n][i];

						// temp[lastTempNodeCreated][secondlastNode] for reverse
						// tracing
						// but only do this if more than 1 temp nodes are
						// created
						if (k > len) {
							tempMatrix[k][k - 1] = -1 * tempMatrix[n][i];
						}

						// temp[origNode][targetNode] = 0 unlinking as they have
						// been
						...
						// ****Even if there is no gap just add a reference for
						// the
						// ****parent to the child for reverse tracing, useful
						// if the
						// ****there is a reversed edge from parent to child and
						// therefore
		...
		// Interconnection matrices at each level, 1 to n-1 after minimizing
		// edges
		// printMatrices(nodeLevels);
	}
	...
	private void removeGapsWithEdgeConcentration(int nodesLevel[]) {
		...
		final int temp = m_nodes.size();
		final int temp2 = graphMatrix[0].length;
		int tempCnt = 1;
				...
				if (graphMatrix[n][i] > 0) {
					if (nodesLevel[i] > nodesLevel[n] + 1) {
						...
						while (tempLevel < nodesLevel[i] - 1) {
							...
							for (; k < graphMatrix.length; k++) {
								if (graphMatrix[tempnode][k] > 0) {
									// System.out.println("tempnode will be
									// true");
						...
						if (tempNodePresent) {
							...
							// and
							// matrix[target][lastknowntempnode]=-original_val
							// for reverse tracing
						...
						if (m > len) {
							...
							// temp[lastTempNodeCreated][secondlastNode] for
							// reverse tracing
							// but only do this if more than 1 temp nodes are
							// created
							tempMatrix[m][m - 1] = -1 * tempMatrix[n][i];
	...
	private int indexOfElementInLevel(int element, int level[]) throws Exception {
		...
		throw new Exception("Error. Didn't find element " + m_nodes.get(element).ID + " in level. Inspect code for "
		...
		for (int i = 0; i < levels.length - 1; i++) {
			...
			for (int j = 0, uidx = 0, lidx = 0; j < (levels[i].length + levels[i + 1].length); j++) {
				if ((j % 2 == 0 && uidx < levels[i].length) || lidx >= levels[i + 1].length) {
					...
					// Deactivating and counting crossings for all edges ending
					// in it
						...
						try {
							do {
								...
								if (levels[i][uidx] == temp.n) {
									k1 += 1;
									k3 += k2;
									// System.out.println("Removing from upper:
									// "+temp.n);
									upper.remove(temp);
								} else {
									k2 += 1;
							...
							System.out.println("levels[i][uidx]: " + levels[i][uidx] + " which is: "
									+ m_nodes.get(levels[i][uidx]).ID + " temp: " + temp + " upper.first: "
									+ upper.first);
							ex.printStackTrace();
							System.exit(-1);
						}
					...
					for (int k = 0; k < levels[i + 1].length; k++) {
						if (edgeOcrnce[levels[i + 1][k]] == 1) {
							...
							// System.out.println("Adding to lower:
							// "+levels[i+1][k]+
							// " which is:
							// "+((GraphNode)m_nodes.get(levels[i+1][k])).ID+
						...
						try {
							do {
								...
								if (levels[i + 1][lidx] == temp.n) {
									...
									k3 += k2;
									lower.remove(temp);
									// System.out.println("Removing from lower:
									// "+temp.n);
								} else {
									k2 += 1;
							...
							System.out.print("levels[i+1][lidx]: " + levels[i + 1][lidx] + " which is: "
									+ m_nodes.get(levels[i + 1][lidx]).ID + " temp: " + temp);
					...
					// Activating all the edges going out towards the upper
					// right
					for (int k = 0; k < n.edges.length; k++) {
						if (n.edges[k][1] < 0) {
							try {
					...
					for (int k = 0; k < levels[i].length; k++) {
						if (edgeOcrnce[levels[i][k]] == 1) {
							...
							// System.out.println("Adding to upper:
							// "+levels[i][k]+
							// " which is : "+
		...
		for (int i = 0; i < colBC.length - 1; i++) {
			if (colBC[i] == colBC[i + 1]) {
				...
				if (crossings(levels) <= crossings(tempLevels)) {
					// System.out.println("Crossings temp:
	...
	public void phaseIIU(final int lindex, final int levels[][]) {
		...
		for (int i = 0; i < rowBC.length - 1; i++) {
			if (rowBC[i] == rowBC[i + 1]) {
				...
				if (crossings(tempLevels) <= crossings(levels)) {
					copy2DArray(tempLevels, levels);
					// System.out.println("Crossings after PhaseID of phaseIIU,
					// in "+
					// "iteration "+i+" of "+(rowBC.length-1)+" at "
					// +lindex+", levels: "+crossings(levels)+
					// " temp: "+crossings(tempLevels));
	...
	protected int lBCenter(int lindex, int eindex, int horPositions[]) {
		...
		if (sum != 0) {
			sum /= lConnectivity(lindex, eindex); // lConectivity;
		}
		return sum;
	}
	...
	protected void priorityLayout1() {
		...
		for (int[] iterator : nodeLevels) {
			int count = 0;
			for (int j = 0; j < iterator.length; j++) {
				horPositions[iterator[j]] = j;
				count++;
			}
			if (count > maxCount) {
				maxCount = count;
	...
	private void priorityLayout2(int level[], int priorities[], int bCenters[], int horPositions[]) {
		...
		for (int iterator : descOrder) {
			for (int iterator1 : descOrder) {
				...
				for (int j = 0; j < priorities.length; j++) {
					if (horPositions[level[iterator1]] > horPositions[level[j]]) {
						leftCount++;
					} else if (horPositions[level[iterator1]] < horPositions[level[j]]) {
						rightCount++;
					}
				}
				...
				// ****Moving left
				while (Math.abs(horPositions[level[iterator1]] - 1 - bCenters[iterator1]) < Math
						.abs(horPositions[level[iterator1]] - bCenters[iterator1])) {

					// ****Checking if it can be moved to left
					int temp = horPositions[level[iterator1]];
					boolean cantMove = false;

					for (int j = leftNodes.length - 1; j >= 0; j--) {
						if (temp - horPositions[level[leftNodes[j]]] > 1) {
							break;
						} else if (priorities[iterator1] <= priorities[leftNodes[j]]) {
							cantMove = true;
							break;
						} else {
					...
					temp = horPositions[level[iterator1]] - 1;
					// ****moving other vertices to left
					for (int j = leftNodes.length - 1; j >= 0; j--) {
						if (temp == horPositions[level[leftNodes[j]]]) {
							// System.out.println("Moving "+
							// ((Node)m_nodes.get(level[leftNodes[j]])).ID+"
							// from "
					...
					horPositions[level[iterator1]] = horPositions[level[iterator1]] - 1;
				}
				...
				while (Math.abs(horPositions[level[iterator1]] + 1 - bCenters[iterator1]) < Math
					...
					for (int rightNode : rightNodes) {
						if (horPositions[level[rightNode]] - temp > 1) {
							break;
						} else if (priorities[iterator1] <= priorities[rightNode]) {
							cantMove = true;
							break;
						} else {
					...
					temp = horPositions[level[iterator1]] + 1;
					for (int iterator2 : rightNodes) {
						if (temp == horPositions[level[iterator2]]) {
							// System.out.println("Moving "+
							// (Node)m_nodes.get(level[rightNodes[j]])).ID+"
							// from "
							// +horPositions[level[rightNodes[j]]]+" to "
							// +(horPositions[level[rightNodes[j]]]+1) );
							horPositions[level[iterator2]] = temp = horPositions[level[iterator2]] + 1;
						}
					}
					// System.out.println("Moving main "+
					// ((GraphNode)m_nodes.get(level[descOrder[i]])).ID+" from "
