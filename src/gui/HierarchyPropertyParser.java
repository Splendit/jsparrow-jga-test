...
public class HierarchyPropertyParser implements Serializable {
	...
	public synchronized void add(String property) {
		...
		if (mRoot.value == null) {
			mRoot.value = values[0];
		}

		buildBranch(mRoot, values, 1);
	}
	...
	private void buildBranch(TreeNode parent, String[] values, int lvl) {
		...
		if (lvl > (mDepth - 1)) {
			mDepth = lvl + 1; // Depth starts from 1
		}
		...
		if (index != -1) {
			...
			added.level = lvl;
			if (parent != mRoot) {
				added.context = parent.context + mSeperator + parent.value;
			} else {
	...
	public String[] tokenize(String rawString) {
		...
		StringTokenizer tk = new StringTokenizer(rawString, mSeperator);
	...
	public boolean contains(String string) {
		...
		if (!item[0].equals(mRoot.value)) {
			return false;
		}

		return isContained(mRoot, item, 1);
	}
	...
	public synchronized boolean goTo(String path) {
		...
		TreeNode old = mCurrent;
		mCurrent = new TreeNode();
		goToRoot();
		String[] nodes = tokenize(path);
		if (!mCurrent.value.equals(nodes[0])) {
			return false;
		}

		for (int i = 1; i < nodes.length; i++) {
			int pos = search(mCurrent.children, nodes[i]);
			if (pos == -1) {
				mCurrent = old;
				return false;
			}
			mCurrent = mCurrent.children.elementAt(pos);
		}
	...
	public synchronized boolean goDown(String path) {
		...
		mCurrent = old.children.elementAt(pos);
		for (int i = 1; i < nodes.length; i++) {
			pos = search(mCurrent.children, nodes[i]);
			if (pos == -1) {
				mCurrent = old;
				return false;
			}
	...
	public synchronized boolean goToChild(String value) {
		if (mCurrent.children == null) {
			return false;
		}

		int pos = search(mCurrent.children, value);
		if (pos == -1) {
			return false;
		}
	...
	public synchronized String[] childrenValues() {
		if (mCurrent.children == null) {
			...
			Vector<TreeNode> kids = mCurrent.children;
	...
	public synchronized String fullValue() {
		if (mCurrent == mRoot) {
			...
			return (mCurrent.context + mSeperator + mCurrent.value);
		}
	}
	...
	public static void main(String args[]) {
		...
		try {
			...
			for (int i = 0; i < chd.length; i++) {
				...
				hpp.goDown(chd[i]);
				System.out.println(
						"real value: " + hpp.getValue() + " | " + hpp.fullValue() + "(level: " + hpp.getLevel() + ")");
				hpp.goToParent();
			}

			System.out.println(
					"Another way to go to root:" + hpp.goTo("node1") + ": " + hpp.getValue() + " | " + hpp.fullValue());
		} catch (Exception e) {
