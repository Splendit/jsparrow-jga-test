 ...
 * the source document. This handler differs from its parent in that it will not
 * replace an existing node with a node from the patch document if that patch
...
public class NonEmptyNodeReplaceInsert extends NodeReplaceInsert {
	...
	protected boolean replaceNode(Node[] primaryNodes, Node testNode, final String attribute, List<Node> usedNodes) {
		...
		for (Node primaryNode : primaryNodes) {
			if (primaryNode.getAttributes().getNamedItem(attribute) != null) {
		...
		Comparator<Node> idCompare = (Node arg0, Node arg1) -> {
