...
/**
 * This handler is responsible for replacing nodes in the source document with
 * the same nodes from the patch document. Note, additional nodes from the patch
 * document that are not present in the source document are simply appended to
 * the source document.
 * 
 * @author jfischer
 *
...
public class NodeReplaceInsert extends BaseHandler {
	...
	private static final Comparator<Node> NODE_COMPARATOR = (Node arg0, Node arg1) -> {
		int response = -1;
	...
	private boolean exhaustedNodesContains(List<Node> exhaustedNodes, Node node) {
		boolean contains = exhaustedNodes.stream()
				.anyMatch(exhaustedNode -> NODE_COMPARATOR.compare(exhaustedNode, node) == 0);
	...
	protected boolean exactNodeExists(Node[] primaryNodes, Node testNode, List<Node> usedNodes) {
		for (Node primaryNode : primaryNodes) {
			if (primaryNode.isEqualNode(testNode)) {
				usedNodes.add(primaryNode);
				return true;
			}
		}
		return false;
	...
	/**
	 * special "replace" method for metatataOverride items having the
	 * "ceilingEntity" attribute specified. Instead of just overwriting a previously
	 * specified item, successive overrideItems with the same ceilingEntity are
	 * "merged" into the previous one; the resulting node contains the logical union
	 * of both old and new children
	 * 
	 * @param primaryNodes
	 * @param testNode
	 * @param usedNodes
