...
public class OptionalIfPresentRule {
	...
	public void default_comment(Optional<String> input) {
		if (input.isPresent()) {
			...
			System.out.println(value);
			if (input.isPresent()) {
				String value2 = input.get(); // comment after value initialization
	...
	public void singleIfBlockBody_savingComments_shouldTransform(Optional<String> input) {
		...
		if /* 1 */ ( /* 2 */input /* 3 */. /* 4 */isPresent/* 5 */(/* 6 */)/* 7 */)/* 8 */ {/* 9.1 */
			/* 9.2 */
			String value /* 11 */ = /* 12 */ input/* 13 */./* 14 */get/* 15 */(/* 16 */)/* 17 */;/* 18 */
			/* 19 */
	...
	public void defaultUseCase_savingComments_shouldTransform(Optional<String> input) {
		...
		if /* 2 */ (/* 3 */input/* 4 */./* 5 */isPresent(/* 6 */)/* 7 */)/* 8 */ {/* 9 */
			/* 10 */
			String value /* 12 */ = /* 13 */ input/* 14 */./* 15 */get/* 16 */ (/* 17 */)/* 18 */;/* 19 */
			/* 20 */
	...
	public void multipleOptionals_shouldTransform(Optional<String> input) {
		String user = Optional.ofNullable("user-name");
		if (input.isPresent()) {
			String value = input.get();
	...
	public void avoidExternalNameConflicts_shouldTransform() {
		...
		String user = Optional.ofNullable(value);
		if (user.isPresent()) {
	...
	public void avoidShadowingFields_shouldTransform() {
		String user = Optional.ofNullable("John Snow");
		if (user.isPresent()) {
	...
	public void nonEffectivelyFinalStringiables_shouldNotTransform(Optional<String> input) {
		int i = 0;
		i++;
		if (input.isPresent()) {
			String value = input.get();
	...
	public void clashingWithPropertyOnQualifiedName_shouldTransform(Optional<String> input) {
		final String user = new IoNonSonoOpzionale();
		if (input.isPresent()) {
			String value = input.get();
			user.value.length();
			System.out.println(value);
	...
	public void discardedSingleOptionalGet_shouldNotTransform() {
		String input = findUserName("");
		if (input.isPresent()) {
			input.get();
			String myString = "somewar";
		}
	}
	...
	public void discardedOptionalGet_shouldTransform() {
		...
		if (input.isPresent()) {
			input.get();
			String myString = input.get();
			findUserName(myString);
		}
	}
