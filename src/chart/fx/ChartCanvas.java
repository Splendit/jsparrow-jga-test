...
public class ChartCanvas extends Canvas implements ChartChangeListener, OverlayChangeListener {
	...
	private boolean hasUniqueID(MouseHandlerFX handler) {
		return this.available_Mouse_Handlers.stream().filter(h -> handler.getID().equals(h.getID())).findFirst()
				.map(h -> false)
				.orElse(this.auxiliary_Mouse_Handlers.stream().noneMatch(h -> handler.getID().equals(h.getID())));
	}
