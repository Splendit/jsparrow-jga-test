...
public class BorderPainter implements Drawable, Serializable {
	...
	/**
	 * Returns the arc height for rounded corners. If this is 0.0, the border will
	 * have square corners. The default value is 0.0.
	 * 
	...
	/**
	 * Returns the arc width for rounded corners. If this is 0.0, the border will
	 * have square corners. The default value is 0.0.
	 * 
	 * @return The arc height.
	 */
	...
	public void draw(Graphics2D g2, Rectangle2D area) {
		...
		if (this.arc_Height > 0.0 && this.arc_Width > 0.0) {
			border = new RoundRectangle2D.Double(0, 0, 0, 0, this.arc_Width, this.arc_Height);
		} else {
			border = new Rectangle2D.Double();
		}
		border.setFrame(area.getX(), area.getY(), area.getWidth() - 1.0, area.getHeight() - 1.0);
	...
	public boolean equals(Object obj) {
		...
		if (this.arc_Height != that.arc_Height) {
			return false;
		}
		if (this.arc_Width != that.arc_Width) {
			return false;
		}
		return true;
	}
	...
	public int hashCode() {
		...
		hash = 83 * hash
				+ (int) (Double.doubleToLongBits(this.arc_Height) ^ (Double.doubleToLongBits(this.arc_Height) >>> 32));
		hash = 83 * hash
				+ (int) (Double.doubleToLongBits(this.arc_Width) ^ (Double.doubleToLongBits(this.arc_Width) >>> 32));
		return hash;
	}
