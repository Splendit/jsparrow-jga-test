...
import dcoref.CorefChain.CorefMention;
import dcoref.Dictionaries.Animacy;
import dcoref.Dictionaries.Gender;
import dcoref.Dictionaries.MentionType;
...
/**
 * Output of (deterministic) coref system. Each CorefChain represents a set of
 * mentions in the text which should all correspond to the same actual entity.
 * There is a representative mention, which stores the best mention of an
 * entity, and then there is a List of all mentions that are coreferent with
 * that mention. The mentionMap maps from pairs of a sentence number and a head
 * word index to a CorefMention. The chainID is an arbitrary integer for the
 * chain number.
 *
 * @author Heeyoung Lee
 */
public class CorefChain implements Serializable {
	...
	public CorefChain(CorefCluster c, Map<String, IntTuple> positions) {
		chainID = c.clusterID;
		// Collect mentions
		mentions = new ArrayList<>();
		mentionMap = new HashMap<>();
		CorefMention represents = null;
		mentions.addAll(c.getCorefMentions().stream().map((m) -> new CorefMention(positions.get(m)))
		...
		for (CorefMention men : mentions) {
			...
			if (!mentionMap.containsKey(position)) {
				mentionMap.put(position, new HashSet<>());
			}
	...
	public CorefChain(int cid, Map<IntPair, Set<CorefMention>> mentionMap, CorefMention representative) {
		...
		mentionMap.values().stream().flatMap(Set::stream).forEach((m) -> this.mentions.add(m));
		Collections.sort(mentions, new CorefMentionComparator());
	}
	...
	/**
	 * get CorefMentions by position (sentence number, headIndex) Can be multiple
	 * mentions sharing headword
	 */
	...
	/**
	 * Return the most representative mention in the chain. Proper mention and a
	 * mention with more pre-modifiers are preferred.
	 */
	...
	/**
	 * Mention for coref output. This is one instance of the entity referred to by a
	 * given CorefChain.
	 */
	public static class CorefMention implements Serializable {
		...
		/**
		 * Position is a binary tuple of (sentence number, mention number in that
		 * sentence). This is used for indexing by mention.
		 */
		...
		/**
		 * This constructor builds the external CorefMention class from the internal
		 * Mention.
		 */
		public CorefMention(IntTuple pos) {
			// index starts from 1
		...
		public String toString() {
			...
			// return "(sentence:" + sentNum + ", startIndex:" + startIndex + "-endIndex:" +
			// endIndex + ")";
		}
