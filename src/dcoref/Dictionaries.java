...
public class Dictionaries {
	...
	private static final Set<String> negations = Collections.unmodifiableSet(new HashSet<>(Arrays.asList("n't", "not",
			"nor", "neither", "never", "no", "non", "any", "none", "nobody", "nothing", "nowhere", "nearly", "almost",
			"if", "false", "fallacy", "unsuccessfully", "unlikely", "impossible", "improbable", "uncertain", "unsure",
			"impossibility", "improbability", "cancellation", "breakup", "lack", "long-stalled", "end", "rejection",
			"failure", "avoid", "bar", "block", "break", "cancel", "cease", "cut", "decline", "deny", "deprive",
			"destroy", "excuse", "fail", "forbid", "forestall", "forget", "halt", "lose", "nullify", "prevent",
			"refrain", "reject", "rebut", "remain", "refuse", "stop", "suspend", "ward")));

	private static final Set<String> neg_relations = Collections
			.unmodifiableSet(new HashSet<>(Arrays.asList("nmod:without", "acl:without", "advcl:without", "nmod:except",
					"acl:except", "advcl:except", "nmod:excluding", "acl:excluding", "advcl:excluding", "nmod:if",
					"acl:if", "advcl:if", "nmod:whether", "acl:whether", "advcl:whether", "nmod:away_from",
					"acl:away_from", "advcl:away_fom", "nmod:instead_of", "acl:instead_of", "advcl:instead_of")));

	private static final Set<String> modals = Collections.unmodifiableSet(new HashSet<>(
	...
	private void setPronouns() {
		animatePronouns.forEach(personPronouns::add);
