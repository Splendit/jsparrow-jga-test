...
public class ConverterUtils implements Serializable, RevisionHandler {
	 ...
	 */
	public final static String CORE_FILE_LOADERS = core.converters.ArffLoader.class.getName();

	/**
	 * the core savers - hardcoded list necessary for RMI/Remote Experiments
	 * (comma-separated list).
	 */
	public final static String CORE_FILE_SAVERS = core.converters.ArffSaver.class.getName();
	...
	public static void initialize() {
		...
		try {
			...
			// generate properties
			// Note: does NOT work with RMI, hence
			// m_FileLoadersCore/m_FileSaversCore
			...
			// loaders
			m_FileLoaders = getFileConverters(props.getProperty(Loader.class.getName(), CORE_FILE_LOADERS),
					new String[] { FileSourcedConverter.class.getName() });

			// URL loaders
			m_URLFileLoaders = getFileConverters(props.getProperty(Loader.class.getName(), CORE_FILE_LOADERS),
					new String[] { FileSourcedConverter.class.getName(), URLSourcedLoader.class.getName() });

			// savers
			m_FileSavers = getFileConverters(props.getProperty(Saver.class.getName(), CORE_FILE_SAVERS),
					new String[] { FileSourcedConverter.class.getName() });
		} catch (Exception e) {
			e.printStackTrace();
			// ignore
		...
		gui.ConverterFileChooser.initDefaultFilters();
	}
	...
	protected static Hashtable<String, String> getFileConverters(Vector<String> classnames, String[] intf) {
		...
		for (String iterator : classnames) {
			classname = iterator;
