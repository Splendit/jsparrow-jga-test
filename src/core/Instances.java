...
public class Instances extends AbstractList<Instance> implements Serializable, RevisionHandler {
	...
	public void deleteAttributeAt(int position) {
		...
		ArrayList<Attribute> newList = new ArrayList<>(m_Attributes.size() - 1);
		HashMap<String, Integer> newMap = new HashMap<>((int) ((m_Attributes.size() - 1) / 0.75));
	...
	public void deleteWithMissing(int attIndex) {
		...
		ArrayList<Instance> newInstances = new ArrayList<>(numInstances());
	...
	public void replaceAttributeAt(/* @non_null@ */Attribute att, int position) {
		...
		if (!(att.name().equals(m_Attributes.get(position).name()))) {
			...
			if ((candidate != null) && (position != candidate.index())) {
				throw new IllegalArgumentException("Attribute name '" + att.name() + "' already in use at position #"
		...
		ArrayList<Attribute> newList = new ArrayList<>(m_Attributes.size());
		HashMap<String, Integer> newMap = new HashMap<>((int) ((m_Attributes.size() + 1) / 0.75));
		for (int i = 0; i < position; i++) {
			Attribute oldAtt = m_Attributes.get(i);
			newList.add(oldAtt);
				...
				if (attribute(attIndex).isNumeric()) {
					if (!instance(i).isMissing(attIndex)) {
						...
						if (Double.isNaN(vars[attIndex])) {
							// For the first value the mean can suffer from loss
							// of precision
							// so we treat it separately and make sure the
						...
						sumWeights[attIndex] = sumWeights[attIndex] + weight;
						means[attIndex] = means[attIndex] + delta / sumWeights[attIndex];
						vars[attIndex] = vars[attIndex] + delta * (value - means[attIndex]);
					}
				}
			}
		}
		...
		for (int attIndex = 0; attIndex < numAttributes(); attIndex++) {
			if (attribute(attIndex).isNumeric()) {
				if (sumWeights[attIndex] <= 1) {
					...
					vars[attIndex] = vars[attIndex] / sumWeights[attIndex] - 1;
					if (vars[attIndex] < 0) {
						vars[attIndex] = 0;
					}
				}
	...
	public/* @pure@ */double variance(int attIndex) {
		...
		for (int i = 0; i < numInstances(); i++) {
			if (!instance(i).isMissing(attIndex)) {
				...
				if (Double.isNaN(var)) {
					// For the first value the mean can suffer from loss of
					// precision
					// so we treat it separately and make sure the calculation
					// stays accurate
					mean = value;
					sumWeights = weight;
					var = 0;
	...
	protected void stratStep(int numFolds) {
		...
		ArrayList<Instance> newVec = new ArrayList<>(m_Instances.size());
		int start = 0;
		int j;
